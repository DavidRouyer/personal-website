// From https://github.com/hapijs/accept
...
export const parse = (
  ...
  const lowers: Map<string, { orig: string, pos: number }> = new Map();
  if (preferences) {
    let pos = 0;
  ...
  for (let i = 0; i < parts.length; ++i) {
    ...
    let token: string = params[0].toLowerCase();
    if (!token) {
      throw new Error(`Invalid ${options.type} header`);
    }

    if (options.equivalents?.has(token)) {
      token = options.equivalents.get(token) as string;
    }
  ...
  for (const selection of values) {
    if (selection === "*") {
      ...
      if (lowers.has(lower)) {
        preferred.push(lowers.get(lower)?.orig as string);
      }
    }
  }
...
const sort = (a: Selection, b: Selection) => {
  const aFirst = -1;
  const bFirst = 1;
